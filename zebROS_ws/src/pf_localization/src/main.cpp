...
int main(int argc, char const *argv[]) {
  ...
  std::pair<double, double> last_pos;
  for (int i = 0; i < 628; i++) {
    // print_particle(pf.get_particles()[0]);
    last_pos = pos;
    pos = std::make_pair(r * cos(theta) + 8, r * sin(theta) + 8);
    std::vector<std::pair<double, double> > measurement;
    Particle p(pos.first, pos.second, 0.5);
    ...
    pf.assign_weights(measurement);
    pf.resample();
    pf.motion_update(sqrt((pos.first - last_pos.first) *
                          (pos.first - last_pos.first) +
                          (pos.second - last_pos.second) *
                          (pos.second - last_pos.second)),
                          0,
                          theta_step);
    pf.set_rotation(theta);
    std::cout << pos.first << ", " << pos.second << ", ";
    print_particle(pf.predict());
    //std::cout << "\n";
    theta += theta_step;
