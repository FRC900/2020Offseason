...
scp -P 22 ~/2020Offseason/scripts/tflite_runtime-1.15.2-cp27-none-linux_armv7l.whl admin@$1:.
...
scp -P 22 ~/2020Offseason/scripts/roscore_roborio.tar.bz2 admin@$1:.
...
scp -P 22 ~/2020Offseason/scripts/ntp-server.conf admin@$1:/etc/ntp.conf

#---------------------------------------------------------------#
#               to setup RTC on the rio
#   Plug the rtc into the i2c port on the rio while unpowered
#
scp -P 22 ~/2020Offseason/scripts/rtc-bq32k.ko admin@$1:.
...
scp -P 22 ~/2020Offseason/scripts/setupClock admin@$1:/etc/init.d/setupClock
...
# Set up ssh keys
scp -P 22 ~/2020Offseason/jetson_setup/roborio_dot_ssh.tar.bz2 admin@$1:.
...
scp -P 22 ~/2020Offseason/scripts/rio_bashrc.sh admin@$1:.
